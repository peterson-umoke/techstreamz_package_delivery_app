...
class CreateRiderOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('rider_orders', function (Blueprint $table) {
            ...
            $table->string('rider_response')->default('pending')->comment('pending, accept, cancel');
            $table->dateTime('rider_response_datetime');
            $table->string('user_response')->default('accept')->comment('accept, cancel');
            ...
            $table->string('admin_response')->default('active')->comment('active, cancel');
